...
function getVehiclesByClassification($classificationName){
    ...
    $sql = "SELECT inv.invId, 
                    ...
                    (SELECT img.imgPath 
                        FROM images img 
                        WHERE inv.invId = img.invId AND img.imgPrimary = 1 LIMIT 1) invImage, 
                    (SELECT img.imgPath 
                        FROM images img 
                        WHERE inv.invId = img.invId AND img.imgPrimary = 1 AND img.imgPath LIKE '%-tn%' LIMIT 1) invThumbnail 
            FROM inventory inv 
